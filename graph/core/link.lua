...
function Link.deserialize(t, graph)
  ...
  -- To może ukryć złe dane (niechcący odwrócone końce). Lepiej wymuś kierunek OUTPUT→INPUT i rzuć błąd, jeśli brak:
  ...
  --Dzięki temu szybciej wykryjemy źle zapisane grafy.
    local fromPort = fromNode:getOutput(fp)
    local toPort   = toNode:getInput(tp)
  if not fromPort or not toPort then
      error(("deserialize: missing ports (from:%s.%s, to:%s.%s)"):format(fid, tostring(fp), tid, tostring(tp)))
  end
